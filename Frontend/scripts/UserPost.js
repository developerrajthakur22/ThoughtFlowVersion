...
function displayAllPost(){
    fetch(`http://thoughflowversion1withaws-env.eba-8prbfeav.ap-south-1.elasticbeanstalk.com/UserPosts/${checkProfidleUserId}`,{
        headers: {
...
function displayFeed(data) {
    ...
    for(let i = data.length-1; i >= 0; i--){
        ...
        if (isLiked) {
            likeButton.innerHTML = `Liked <i class="fa-solid fa-heart"></i> (${likeCount})`;
            likeButton.setAttribute("your-action", "liked");
        } else {
            likeButton.innerHTML = `Like <i class="fa-regular fa-heart fa-beat"></i>  (${likeCount})`;
